...
def analysisCyberARMOutPut():
    ...
    ta_sc_mapping()
    recommendedCDM = readResultFileWithName('ResourceFolder/ResultFiles/OutResult_500_0_0.8_0.txt')
    ...
    for asset_risk in recommendedCDM[1]:
        if asset_risk['asset_name'] not in asset_threat_action:
            continue
        if asset_risk['asset_name'] not in asset_ta_risk:
            asset_ta_risk[asset_risk['asset_name']] = []
        asset_ta_risk[asset_risk['asset_name']].append(asset_risk['threat_list'])
...
def generateOutput():
    ...
    print("Asset TA Statistics %s"%(asset_ta_risk))
